...
if ( !function_exists( 'healthandcare_template_services_4_theme_setup' ) ) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_template_services_4_theme_setup', 1 );
	function healthandcare_template_services_4_theme_setup() {
		healthandcare_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'healthandcare')
		));
	}
}

// Template output
if ( !function_exists( 'healthandcare_template_services_4_output' ) ) {
	function healthandcare_template_services_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				echo healthandcare_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
