<?php
/**
 * HealthandCARE Framework: social networks
 *
...
// Theme init
if (!function_exists('healthandcare_socials_theme_setup')) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_socials_theme_setup' );
	function healthandcare_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'healthandcare_facebook_og_tags', 5 );
		}
...
$HEALTHANDCARE_GLOBALS['share_links'] = array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=healthandcare',
...
if (!function_exists('healthandcare_add_share_link')) {
	function healthandcare_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $HEALTHANDCARE_GLOBALS;
		$HEALTHANDCARE_GLOBALS['share_links'] = array_merge( $HEALTHANDCARE_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('healthandcare_show_share_links')) {
	function healthandcare_show_share_links($args) {
		if ( healthandcare_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => healthandcare_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => healthandcare_get_custom_option('show_share'),				// share block direction
			'caption' => healthandcare_get_custom_option('share_caption'),				// share block caption
			'share' => healthandcare_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = empty($soc['url']) && !empty($HEALTHANDCARE_GLOBALS['share_links'][$sn]) ? $HEALTHANDCARE_GLOBALS['share_links'][$sn] : $soc['url'];
				...
				$output .= '<div class="sc_socials_item">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(healthandcare_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('healthandcare_prepare_socials')) {
	function healthandcare_prepare_socials($list, $style='') {
		if (empty($style)) $style = healthandcare_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || healthandcare_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : healthandcare_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $sn;
				} else {
...
if (!function_exists('healthandcare_get_twitter_data')) {
	function healthandcare_get_twitter_data($cfg) {
		$data = get_transient("twitter_data_".($cfg['mode']));
		if (!$data) {
			require_once( healthandcare_get_file_dir('lib/tmhOAuth/tmhOAuth.php') );
			...
			$code = $tmhOAuth->user_request(array(
				'url' => $tmhOAuth->url(healthandcare_get_twitter_mode_url($cfg['mode']))
			));
			if ($code == 200) {
				$data = json_decode($tmhOAuth->response['response'], true);
				if (isset($data['status'])) {
					$code = $tmhOAuth->user_request(array(
						'url' => $tmhOAuth->url(healthandcare_get_twitter_mode_url($cfg['oembed'])),
						'params' => array(
		...
		} else if (!is_array($data) && healthandcare_substr($data, 0, 2)=='a:') {
...
if (!function_exists('healthandcare_get_twitter_mode_url')) {
	function healthandcare_get_twitter_mode_url($mode) {
		$url = '/1.1/statuses/';
...
if (!function_exists('healthandcare_prepare_twitter_text')) {
	function healthandcare_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('healthandcare_get_twitter_followers')) {
	function healthandcare_get_twitter_followers($cfg) {
		$data = healthandcare_get_twitter_data($cfg);
		return $data && isset($data[0]['user']['followers_count']) ? $data[0]['user']['followers_count'] : 0;
	}
	// Old version
	/*
	function healthandcare_get_twitter_followers($account) {
		$tw = get_transient("twitterfollowers");
		if ($tw !== false) return $tw;
		$tw = '?';
		$url = esc_url('https://twitter.com/users/show/'.($account));
		$headers = get_headers($url);
		if (healthandcare_strpos($headers[0], '200')) {
			$xml = healthandcare_fgc($url);
...
if (!function_exists('healthandcare_get_facebook_likes')) {
	function healthandcare_get_facebook_likes($account) {
...
function healthandcare_facebook_og_tags() {
	global $post;
	if ( !is_singular() || healthandcare_get_global('blog_streampage')) return;
...
if (!function_exists('healthandcare_get_feedburner_counter')) {
	function healthandcare_get_feedburner_counter($account) {
