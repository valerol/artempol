...
function healthandcare_skin_customizer(option, val) {
	...
	if (false && option == 'body_style') {
		...
		var params = HEALTHANDCARE_GLOBALS['co_add_params']!=undefined ? HEALTHANDCARE_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#healthandcare-customizer-styles-'+option).length > 0 ? jQuery('#healthandcare-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="healthandcare-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
