...
if ( !function_exists( 'healthandcare_template_testimonials_2_theme_setup' ) ) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_template_testimonials_2_theme_setup', 1 );
	function healthandcare_template_testimonials_2_theme_setup() {
		healthandcare_add_template(array(
			...
			'title'  => esc_html__('Testimonials /Style 2/', 'healthandcare')
		));
	}
}

// Template output
if ( !function_exists( 'healthandcare_template_testimonials_2_output' ) ) {
	function healthandcare_template_testimonials_2_output($post_options, $post_data) {
		$show_title = true;
			...
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo ($post_options['tag_css'] ? ' style="'.esc_attr($post_options['tag_css']).'"' : '');?>>
				...
				<div class="sc_testimonial_author"><?php 
					echo ($post_options['link'] 
							...
							: '<div class="sc_testimonial_author_name">'.($post_options['author']).'</div>')
						. ($post_options['position']
							? '<div class="sc_testimonial_author_position">'.($post_options['position']).'</div>'
							: ''); ?></div>
				<?php } ?>
			</div>
		<?php
