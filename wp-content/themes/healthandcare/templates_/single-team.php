...
if ( !function_exists( 'healthandcare_template_single_team_theme_setup' ) ) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_template_single_team_theme_setup', 1 );
	function healthandcare_template_single_team_theme_setup() {
		healthandcare_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'healthandcare'),
...
// Template output
if ( !function_exists( 'healthandcare_template_single_team_output' ) ) {
	function healthandcare_template_single_team_output($post_options, $post_data) {
		$post_data['post_views']++;
		$show_title = healthandcare_get_custom_option('show_post_title')=='yes';
		...
		healthandcare_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
