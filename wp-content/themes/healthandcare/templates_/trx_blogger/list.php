...
if ( !function_exists( 'healthandcare_template_list_theme_setup' ) ) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_template_list_theme_setup', 1 );
	function healthandcare_template_list_theme_setup() {
		healthandcare_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'healthandcare')
			));
	}
}

// Template output
if ( !function_exists( 'healthandcare_template_list_output' ) ) {
	function healthandcare_template_list_output($post_options, $post_data) {
