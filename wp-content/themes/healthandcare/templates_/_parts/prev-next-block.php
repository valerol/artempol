...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = healthandcare_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo ($page_style=='single-portfolio-fullscreen' ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''); ?>>
			...
			$args = healthandcare_query_add_posts_and_cats($args, '', $post_data['post_type'],
			...
			if ( $prev ) {
				...
				$desc = healthandcare_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = healthandcare_strshort($next->post_title, 30);
				?>
