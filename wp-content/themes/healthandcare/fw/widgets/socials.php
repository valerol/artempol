 ...
 */
add_action( 'widgets_init', 'healthandcare_widget_socials_load' );

/**
 * Register our widget.
 */
function healthandcare_widget_socials_load() {
	register_widget( 'healthandcare_widget_socials' );
}

/**
 * flickr Widget class.
 */
class healthandcare_widget_socials extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'healthandcare') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'healthandcare_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'healthandcare_widget_socials', esc_html__('HealthandCARE - Show logo and social links', 'healthandcare'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? healthandcare_do_shortcode($instance['text']) : '';
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_image=='')	$logo_image = $HEALTHANDCARE_GLOBALS['logo'];
					if ($logo_text=='')		$logo_text = $HEALTHANDCARE_GLOBALS['logo_text'];
					if ($logo_slogan=='')	$logo_slogan = $HEALTHANDCARE_GLOBALS['logo_slogan'];
				...
				if (!empty($text)) {
					?>
					<div class="logo_descr"><?php echo nl2br(healthandcare_do_shortcode($text)); ?></div>
                    <?php
				}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => esc_html__('Show logo and social icons', 'healthandcare') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e('Description:', 'healthandcare'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php wp_kses(__('Logo image:<br />(if empty - use logo from Theme Options)', 'healthandcare'), $HEALTHANDCARE_GLOBALS['allowed_tags']); ?></label>
            ...
            <?php
			echo trim(healthandcare_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php esc_html_e('Logo text:', 'healthandcare'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show logo:', 'healthandcare'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show', 'healthandcare'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php esc_html_e('Hide', 'healthandcare'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show social icons:', 'healthandcare'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show', 'healthandcare'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php esc_html_e('Hide', 'healthandcare'); ?></label>
		</p>
