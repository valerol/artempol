	...
	function healthandcare_sc_button($atts, $content=null){
		...
		$output = '<a href="' . (empty($link) ? '#' : $link) . '"'
			...
			. ' class="sc_button sc_button_' . esc_attr($type) 
					...
					. (healthandcare_param_is_on($popup) ? ' popup_link' : '')
					. '"'
			. ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. '>'
...
if (!function_exists('healthandcare_sc_contact_form')) {
	function healthandcare_sc_contact_form($atts, $content = null) {
		...
		$output .= '<form' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' data-formtype="'.($content ? 'custom' : 'contact').'" method="post" action="' . esc_url($action ? $action : $HEALTHANDCARE_GLOBALS['ajax_url']) . '">'
					. ($content != '' && healthandcare_param_is_on($custom)
						...
						: '<div class="sc_contact_form_info">'
								...
								.'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_subj">' . esc_html__('Subject', 'healthandcare') . '</label><input id="sc_contact_form_subj" type="text" name="subject" placeholder="' . esc_html__('Subject', 'healthandcare') . '"></div>'
							.'</div>'
...
if ( !function_exists( 'healthandcare_sc_contact_form_send' ) ) {
	function healthandcare_sc_contact_form_send() {
		...
		if (!($contact_email = healthandcare_get_theme_option('contact_email')) && !($contact_email = healthandcare_get_theme_option('admin_email')))
			...
			if ($type=='contact') {
				$user_name	= healthandcare_strshort($post_data['username'],	100);
				$user_email	= healthandcare_strshort($post_data['email'],	100);
				$user_subj	= healthandcare_strshort($post_data['subject'],	100);
				$user_msg	= healthandcare_strshort($post_data['message'],	healthandcare_get_theme_option('message_maxlength_contacts'));
				...
				$msg = "\n".esc_html__('Name:', 'healthandcare')   .' '.esc_html($user_name)
					...
					.  "\n".esc_html__('Subject:', 'healthandcare').' '.esc_html($user_subj)
					.  "\n".esc_html__('Message:', 'healthandcare').' '.esc_html($user_msg);
	...
	function healthandcare_sc_googlemap($atts, $content = null) {
		...
		foreach ($HEALTHANDCARE_GLOBALS['sc_googlemap_markers'] as $marker) {
			...
			$output .= '<div id="'.esc_attr($marker['id']).'" class="sc_googlemap_marker"'
				...
				. ' data-description="'.esc_attr(healthandcare_strmacros($marker['description'])).'"'
