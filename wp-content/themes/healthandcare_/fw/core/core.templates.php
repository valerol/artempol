<?php
/**
 * HealthandCARE Framework: templates and thumbs management
 *
...
// Theme init
if (!function_exists('healthandcare_templates_theme_setup')) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_templates_theme_setup' );
	function healthandcare_templates_theme_setup() {

		// Add custom thumb sizes into media manager
		add_filter( 'image_size_names_choose', 'healthandcare_show_thumb_sizes');
	}
}
...
if (!function_exists('healthandcare_add_template')) {
	function healthandcare_add_template($tpl) {
		...
		if (!isset($HEALTHANDCARE_GLOBALS['registered_templates'])) $HEALTHANDCARE_GLOBALS['registered_templates'] = array();
		$HEALTHANDCARE_GLOBALS['registered_templates'][$tpl['layout']] = $tpl;
		if (!empty($tpl['thumb_title']))
			healthandcare_add_thumb_sizes( $tpl );
		else 
			$tpl['thumb_title'] = '';
	}
}
...
if (!function_exists('healthandcare_get_template_name')) {
	function healthandcare_get_template_name($layout_name) {
		global $HEALTHANDCARE_GLOBALS;
		return $HEALTHANDCARE_GLOBALS['registered_templates'][$layout_name]['template'];
	}
}

// Return true, if template required content
if (!function_exists('healthandcare_get_template_property')) {
	function healthandcare_get_template_property($layout_name, $what) {
		global $HEALTHANDCARE_GLOBALS;
		return !empty($HEALTHANDCARE_GLOBALS['registered_templates'][$layout_name][$what]) ? $HEALTHANDCARE_GLOBALS['registered_templates'][$layout_name][$what] : '';
	}
}

// Return template output function name
if (!function_exists('healthandcare_get_template_function_name')) {
	function healthandcare_get_template_function_name($layout_name) {
		global $HEALTHANDCARE_GLOBALS;
		return 'healthandcare_template_'.str_replace(array('-', '.'), '_', $HEALTHANDCARE_GLOBALS['registered_templates'][$layout_name]['template']).'_output';
	}
}
...
if (!function_exists('healthandcare_add_thumb_sizes')) {
	function healthandcare_add_thumb_sizes($sizes) {
		global $HEALTHANDCARE_GLOBALS;
		if (!isset($sizes['h_crop']))		$sizes['h_crop'] =  isset($sizes['h']) ? $sizes['h'] : null;
		//if (empty($sizes['mode']))			$sizes['mode'] = 'blog';
		if (empty($sizes['thumb_title']))	$sizes['thumb_title'] = healthandcare_strtoproper($sizes['layout']);
		$thumb_slug = healthandcare_get_slug($sizes['thumb_title']);
		if (empty($HEALTHANDCARE_GLOBALS['thumb_sizes'][$thumb_slug])) {
			if (empty($HEALTHANDCARE_GLOBALS['thumb_sizes'])) $HEALTHANDCARE_GLOBALS['thumb_sizes'] = array();
			$HEALTHANDCARE_GLOBALS['thumb_sizes'][$thumb_slug] = $sizes;
			...
			if ($sizes['h']!=$sizes['h_crop']) {
				...
				//$sizes['title'] .= esc_html__(' (cropped)', 'healthandcare');
				//$HEALTHANDCARE_GLOBALS['thumb_sizes'][$sizes['layout'].'_crop'] = $sizes;
...
if (!function_exists('healthandcare_get_thumb_sizes')) {
	function healthandcare_get_thumb_sizes($opt) {
		$opt = array_merge(array(
			'layout' => 'excerpt'
		), $opt);
		global $HEALTHANDCARE_GLOBALS;
		$thumb_slug = empty($HEALTHANDCARE_GLOBALS['registered_templates'][$opt['layout']]['thumb_title']) ? '' : healthandcare_get_slug($HEALTHANDCARE_GLOBALS['registered_templates'][$opt['layout']]['thumb_title']);
		$rez = $thumb_slug ? $HEALTHANDCARE_GLOBALS['thumb_sizes'][$thumb_slug] : array('w'=>null, 'h'=>null, 'h_crop'=>null);
		return $rez;
	}
}

// Show custom thumb sizes into media manager sizes list
if (!function_exists('healthandcare_show_thumb_sizes')) {
	function healthandcare_show_thumb_sizes( $sizes ) {
		global $HEALTHANDCARE_GLOBALS;
		$thumb_sizes = $HEALTHANDCARE_GLOBALS['thumb_sizes'];
