...
if ( !function_exists( 'healthandcare_template_services_2_theme_setup' ) ) {
	add_action( 'healthandcare_action_before_init_theme', 'healthandcare_template_services_2_theme_setup', 1 );
	function healthandcare_template_services_2_theme_setup() {
		healthandcare_add_template(array(
			'layout' => 'services-2',
			'template' => 'services-2',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 2/', 'healthandcare'),
...
// Template output
if ( !function_exists( 'healthandcare_template_services_2_output' ) ) {
	function healthandcare_template_services_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = healthandcare_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
